...
contract GmxAdapter is YakAdapter {
    ...
    function setPoolTokens() public {
        uint256 whitelistedTknsLen = IGmxVault(vault)
    ...
    function getPrices(address _tokenIn, address _tokenOut)
        internal
        view
    ...
    function isWithinVaultLimits(
        ...
        uint256 newUsdgAmount = IGmxVault(vault).usdgAmounts(_tokenIn).add(
            _amountInUsdg
        );
        ...
        if (
            reservedAmountNotExceeded &&
            bufferAmountNotExceeded &&
            maxDebtNotExceeded
        ) {
    ...
    function _query(
        ...
        uint256 feeBasisPoints = IGmxVault(vault)
            .vaultUtils()
            .getSwapFeeBasisPoints(_tokenIn, _tokenOut, usdgAmount);
        uint256 amountOutAfterFees = _amountOut.mul(
            BASIS_POINTS_DIVISOR.sub(feeBasisPoints)
