...
module.exports.topUpAccountWithToken = async (topper, recieverAddress, tokenAddress, amount, routerContract) => {
  ...
  return routerContract
    .connect(topper)
    .swapAVAXForExactTokens(amount, [_WAVAX_, tokenAddress], recieverAddress, parseInt(Date.now() / 1e3) + 3000, {
      value: topperBalance.div('2'),
    })
    .then((response) => response.wait())
}

module.exports.getTokenContract = (tokenAddress) =>
  ethers.getContractAt('contracts/interface/IWETH.sol:IWETH', tokenAddress)
