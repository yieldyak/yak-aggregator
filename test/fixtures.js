...
const _xjoeAdapter = async () => {
  ...
  const [XJoe, XJoeAdapter] = await Promise.all([
...
const _kyberAdapter = async () => {
  ...
  const [KyberRouter, KyberAdapter] = await Promise.all([
    ethers.getContractAt('IKyberRouter', unilikeRouters.kyber),
    KyberAdapterFactory.then((f) =>
      f.deploy(
        'KyberAdapter',
        [
...
const _platypusAdapter = async () => {
  ...
  const [PlatypusV1, PlatypusAdapter] = await Promise.all([
    ethers.getContractAt('IPlatypus', curvelikePools.PlatypusV1),
    PlatypusAdapterFactory.then((f) => f.deploy('PlatypusV1Adapter', curvelikePools.PlatypusV1, 5.14e5)),
  ])
  return {
    PlatypusAdapter,
    PlatypusV1,
...
const _axialAdapter = async () => {
  ...
  const [
    ...
    CurvelikeMetaAdapterFactory.then((f) => f.deploy('AxialAM3DUSDCAdapter', curvelikePools.AxialAM3DUSDC, 6.5e5)),
    CurveLikeAdapterFactory.then((f) => f.deploy('AxialAM3DAdapter', curvelikePools.AxialAM3D, 3.6e5)),
    CurveLikeAdapterFactory.then((f) => f.deploy('AxialAC4DAdapter', curvelikePools.AxialAC4D, 3.6e5)),
    CurveLikeAdapterFactory.then((f) => f.deploy('AxialAA3DAdapter', curvelikePools.AxialAA3D, 3.6e5)),
    CurveLikeAdapterFactory.then((f) => f.deploy('AxialAS4DAdapter', curvelikePools.AxialAS4D, 3.6e5)),
  ])
  return {
    AxialAM3DUSDC,
    AxialAM3D,
...
const _synapseAdapter = async () => {
  const CurvelikeAdapterFactory = ethers.getContractFactory('CurveLikeAdapter')
  const [SynapsePool, SynapseAdapter] = await Promise.all([
    ethers.getContractAt('ICurveLikePool', curvelikePools.SynapseDAIeUSDCeUSDTeNUSD),
    CurvelikeAdapterFactory.then((f) =>
      f.deploy('Synapse YakAdapter', curvelikePools.SynapseDAIeUSDCeUSDTeNUSD, 3.6e5)
    ),
  ])
  return {
    SynapseAdapter,
