...
describe('YakAdapter - Pangolin - Queries', function () {
  ...
  it('Query-exact-path should match underlying router', async () => {
    ...
    // Query adapter
    let amountOutAdapter = await PangolinAdapter.query(amountIn, tokenIn, tokenOut)
    // Query dex router
    let amountOutRouter = await PangolinRouter.getAmountsOut(amountIn, [tokenIn, tokenOut]).then(
  ...
  it('Query tokens should match underlying router', async () => {
    // Options
    let amountIn = ethers.utils.parseUnits('100')
    let path = [tkns.WAVAX.address, tkns.DAI.address]
    // Query adapter
    let amountOutAdapter = await PangolinAdapter.query(amountIn, path[0], path[1])
    // Query dex router
    let amountOutRouter = await PangolinRouter.getAmountsOut(amountIn, path).then(
      (amounts) => amounts[amounts.length - 1]
    )
    // Compare
    expect(amountOutAdapter).to.equal(amountOutRouter)
  ...
  it('Query tokens should work despite invalid path (tokenIn)', async () => {
    ...
    amountsOutRouter.push(
      await PangolinRouter.getAmountsOut(amountIn, path).then((amounts) => amounts[amounts.length - 1])
    )
  ...
  it('Query tokens should work despite invalid path (tokenOut)', async () => {
    // Options
    let amountIn = ethers.utils.parseUnits('100')
    let path = [tkns.DAI.address, tkns.WAVAX.address]
  ...
  it('Query should return zero if there is no path between tokens', async () => {
    // Options
    let path = [tkns.DAI.address, tkns.FRAX.address]
    let amountIn = ethers.utils.parseUnits('1000')
    // Query adapter
    let amountOutAdapter = await PangolinAdapter.query(amountIn, path[0], path[1])
    // Querying original router should cause a revert
    await expect(PangolinRouter.getAmountsOut(amountIn, path).then((amounts) => amounts[amounts.length - 1])).to
      .reverted
  ...
  it('Query should return zero if start and end token are the same', async () => {
    // Options
    let path = [tkns.WAVAX.address, tkns.WAVAX.address]
