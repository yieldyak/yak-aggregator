...
describe('YakAdapter Swaps', function () {
  ...
  it('Swap returned amount should match underlying router quote', async () => {
    ...
    // Query
    let amountOutAdapter = await ZeroAdapter.query(amountIn, tokenFrom.address, tokenTo.address)
    // Swap
    ...
    const swap = () =>
      ZeroAdapter.connect(trader).swap(amountIn, amountOutAdapter, tokenFrom.address, tokenTo.address, trader.address)
    // Swap and compare
