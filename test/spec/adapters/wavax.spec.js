...
describe('YakAdapter - wAVAX', function () {
  ...
  before(async () => {
    ...
    YakRouter = await ethers
      .getContractFactory('YakRouter', { libraries: { BytesManipulation: BytesManipulationV0.address } })
  ...
  describe('swap', async () => {
    it('Swapping is 1:1 when AVAX-WAVAX', async () => {
      ...
      // Swapping
      const swap = () =>
        YakRouter.swapNoSplitFromAVAX([amountIn, amountIn, queryRes.path, queryRes.adapters], trader.address, 0, {
          value: amountIn,
        })
    ...
    it('Swapping is 1:1 when WAVAX-AVAX', async () => {
      ...
      // Swapping
      const swap = () =>
        YakRouter.connect(trader).swapNoSplitToAVAX(
          [amountIn, amountIn, queryRes.path, queryRes.adapters],
          trader.address,
          0
        )
      // Check that swap matches the query
    ...
    it('Check gas cost', async () => {
      ...
      // Swapping
      const swapAVAX2WAVAXReceipt = await YakRouter.connect(trader)
        .swapNoSplitFromAVAX([amountIn, amountIn, queryRes.path, queryRes.adapters], trader.address, 0, {
          value: amountIn,
        })
        .then((r) => r.wait())
      await tknFrom.connect(trader).approve(YakRouter.address, amountIn)
      const swapWAVAX2AVAXReceipt = await YakRouter.connect(trader)
        .swapNoSplitToAVAX([amountIn, amountIn, queryRes.path, queryRes.adapters], trader.address, 0)
        .then((r) => r.wait())
      const swapGas = Math.max(parseInt(swapAVAX2WAVAXReceipt.gasUsed), parseInt(swapWAVAX2AVAXReceipt.gasUsed))
