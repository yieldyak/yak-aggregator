...
describe('YakAdapter - Axial', function () {
  ...
  describe('AM3DUSDC', async () => {
    ...
    it('Adapter supports USDCe, MIM, DAIe, USDC', async () => {
      const supportedTokens = [assets.USDCe, assets.USDC, assets.DAIe, assets.MIM]
    ...
    it('Querying adapter matches the price from original contract', async () => {
      ...
      const [tknFromIndex, tknToIndex] = await Promise.all([Adapter.tokenIndex(tknFrom), Adapter.tokenIndex(tknTo)])
  ...
  describe('AM3D', async () => {
    ...
    it('Adapter supports USDCe, MIM, DAIe', async () => {
      const supportedTokens = [assets.USDCe, assets.DAIe, assets.MIM]
      for (let tkn of supportedTokens) {
        expect(await Adapter.isPoolToken(tkn)).to.be.true
      }
    })
  ...
  describe('AC4D', async () => {
    ...
    it('Adapter supports TSD, MIM, DAIe, FRAX(canonical)', async () => {
      const supportedTokens = [assets.TSD, assets.FRAXc, assets.MIM, assets.DAIe]
      for (let tkn of supportedTokens) {
        expect(await Adapter.isPoolToken(tkn)).to.be.true
      }
    })
  ...
  describe('AA3D', async () => {
    ...
    it('Adapter supports AVAI, MIM, USDCe', async () => {
      const supportedTokens = [assets.AVAI, assets.MIM, assets.USDCe]
      for (let tkn of supportedTokens) {
        expect(await Adapter.isPoolToken(tkn)).to.be.true
      }
    })
