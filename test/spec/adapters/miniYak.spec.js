...
describe('YakAdapter - MiniYak', function () {
  ...
  describe('query', async () => {
    it('mYAK/YAK should be 1M', async () => {
      ...
      expect(await MiniYakAdapter.query(amountIn, tknIn.address, tknOut.address)).to.equal(amountIn)
    })
    ...
    it('Return 0 if swap isnt YAK-mYAK or mYAK-YAK', async () => {
      ...
      const tknOut = mYAK
      const amountIn = parseUnits('1', await tknIn.decimals())
      expect(await MiniYakAdapter.query(amountIn, tknIn.address, tknOut.address)).to.equal(0)
    })
  })
  ...
  describe('swap', async () => {
    ...
    it('Revert if swap isnt YAK-mYAK or mYAK-YAK', async () => {
      await expect(MiniYakAdapter.swap('0', '0', YAK.address, YAK.address, trader.address)).to.revertedWith(
        'MiniYakAdapter: Unsupported token'
      )
    })
    ...
    it('mYak/Yak should be 1M - moon', async () => {
      ...
      const swap = () => MiniYakAdapter.swap(amountIn, '0', tknIn.address, tknOut.address, trader.address)
      await expect(swap).to.changeTokenBalance(mYAK, trader, amountIn)
    })
    ...
    it('YAK/mYAK should be 1μ - unmoon', async () => {
      ...
      await tknIn.transfer(MiniYakAdapter.address, amountIn)
      const swap = () => MiniYakAdapter.swap(amountIn, '0', tknIn.address, tknOut.address, trader.address)
      await expect(swap).to.changeTokenBalance(YAK, trader, amountIn)
    })
  })
})
