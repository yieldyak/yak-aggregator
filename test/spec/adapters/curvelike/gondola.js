...
describe('YakAdapter - Gondola', function () {
  ...
  describe('GondolaDAI', () => {
    ...
    it('Swapping matches query (small num)', async () => {
      ...
      // Comparing results
      expect(traderRecvTknBalAfter.sub(traderRecvTknBalBefore)).to.equal(amountOutAdapter)
    })
  ...
  describe('GondolaUSDT', () => {
    ...
    it('Swapping matches query', async () => {
      ...
      // Swapping
      await expect(
        adapters.GondolaUSDTAdapter.connect(trader).swap(
          amountIn,
          amountOutAdapter,
          tokenFrom.address,
          tokenTo.address,
  ...
  describe('GondolaUSDCeUSDTe', () => {
    it('Querying adapter returns zero if there is an error', async () => {
      ...
      // Querying adapter
      expect(await adapters.GondolaUSDTeUSDCeAdapter.query(amountIn, tokenFrom.address, tokenTo.address)).to.equal('0')
      // Querying original contract
      ...
      await expect(pools.GondolaUSDTeUSDCe.calculateSwap(fromIndex, toIndex, amountIn)).to.revertedWith(
