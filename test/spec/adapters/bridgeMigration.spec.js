...
describe('YakAdapter BridgeMigration', function () {
  ...
  describe('query', async () => {
    it('Return 0 for 0', async () => {
      ...
      expect(await BMAdapter.query(amountIn, tokenIn.address, tokenOut.address)).to.equal('0')
    })
    ...
    it('Valid swap returns input amount', async () => {
      ...
      const tokenIn = oldTokens[0]
      const tokenOut = bridgeTokens[0]
      expect(await BMAdapter.query(amountIn, tokenIn.address, tokenOut.address)).to.equal(amountIn)
    })
  })
  ...
  describe('setTokens', async () => {
    it('Revert if src and dest tokens dont match', async () => {
      ...
      await expect(BMAdapterFactory.deploy(_bridgeTokens, _oldTokens, 9e4)).to.revertedWith(
        'BridgeMigrationAdapter: Invalid combination'
      )
    })
    it('Approve when tokens are set and allowance is not max', async () => {
      ...
      expect(await oldTokens[0].allowance(_adapter.address, bridgeTokens[0].address)).to.equal(
  ...
  describe('swap', async () => {
    it('Successful swap', async () => {
      ...
      const tokenOut = bridgeTokens[0]
      const swap = () =>
        BMAdapter.connect(trader).swap(inputAmount, inputAmount, tokenIn.address, tokenOut.address, trader.address)
