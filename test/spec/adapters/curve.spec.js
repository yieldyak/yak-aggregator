...
describe('YakAdapter - Curve', function () {
  ...
  describe('aave', async () => {
    ...
    it('Adapter supports USDC, USDT, DAI', async () => {
      const supportedTokens = [assets.USDCe, assets.USDTe, assets.DAIe]
    ...
    it('Querying adapter matches the price from original contract', async () => {
      ...
      const amountOutOriginal = await Original['get_dy_underlying(int128,int128,uint256)'](
        tknIndexFrom,
        tknIndexTo,
        amountIn
      )
    ...
    it('Check gas cost', async () => {
      ...
      for (let [tokenFrom, tokenTo] of options) {
        ...
        const swapGas = await Adapter.connect(trader)
          .swap(amountIn, 1, tokenFrom.address, tokenTo.address, trader.address)
          .then((tr) => tr.wait())
  ...
  describe('atricrypto', async () => {
    ...
    it('Adapter supports WBTC, WETH, USDC, USDT, DAI', async () => {
      const supportedTokens = [assets.WBTCe, assets.WETHe, assets.USDCe, assets.USDTe, assets.DAIe]
      for (let tkn of supportedTokens) {
        expect(await Adapter.isPoolToken(tkn)).to.be.true
      }
    })
    ...
    it('Querying adapter matches the price from original contract within 4bps', async () => {
      ...
      const amountIn = parseUnits('1', tknFromDecimals)
      // Query original contract
      const amountOutOriginal = await Original['get_dy_underlying(uint256,uint256,uint256)'](
        tknIndexFrom,
        tknIndexTo,
        amountIn
      )
  ...
  describe('ren', async () => {
    ...
    it('Adapter supports renBTC, WBTCe', async () => {
      const supportedTokens = [assets.renBTC, assets.WBTCe]
      for (let tkn of supportedTokens) {
        expect(await Adapter.isPoolToken(tkn)).to.be.true
      }
    })
  ...
  describe('3poolV2', async () => {
    ...
    it('Adapter supports MIM, USDTe, USDCe', async () => {
      const supportedTokens = [assets.MIM, assets.USDTe, assets.USDCe]
      for (let tkn of supportedTokens) {
        expect(await Adapter.isPoolToken(tkn)).to.be.true
      }
    })
    ...
    it('Querying adapter matches the price from original contract', async () => {
      ...
      const tknIndexFrom = 0
      const tknIndexTo = 1
      const [tknFrom, tknTo] = await Promise.all([Original.coins(tknIndexFrom), Original.coins(tknIndexTo)])
      const tknFromDecimals = await getTokenContract(tknFrom).then((t) => t.decimals())
  ...
  describe('mim', async () => {
    ...
    it('Adapter supports MIM, USDTe, USDCe, DAIe', async () => {
      const supportedTokens = [assets.MIM, assets.USDTe, assets.USDCe, assets.DAIe]
      for (let tkn of supportedTokens) {
        expect(await Adapter.isPoolToken(tkn)).to.be.true
      }
    })
