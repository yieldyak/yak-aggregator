...
describe('Yak Router - query', () => {
  ...
  it('Return the best option for trade between adapters', async () => {
    ...
    for (let i = 0; i < adapterCount; i++) {
      let result = await YakRouter.queryAdapter(amountIn, tokenIn, tokenOut, i)
      options.push(result)
    }
    let bestOptionQuery = await YakRouter['queryNoSplit(uint256,address,address)'](amountIn, tokenIn, tokenOut)
  ...
  it('Returning best path with gas should return gasEstimate', async () => {
    ...
    let steps = 3
    let result = await YakRouter.findBestPathWithGas(amountIn, tokenIn, tokenOut, steps, gasPrice, { gasLimit: 1e9 })
    // Expect to find a path
    expect(result.amounts[result.amounts.length - 1]).to.gt('0')
    // Expect first token in the path to be token from
    expect(result.path[0]).to.equal(tokenIn)
