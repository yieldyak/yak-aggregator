...
describe('Yak Router - swap', () => {
  ...
  it('Router swap matched the query - multiple hops', async () => {
    ...
    let result = await YakRouter.findBestPath(amountIn, tokenIn, tokenOut, steps)
    ...
    await YakRouter.connect(trader).swapNoSplit(
      [result.amounts[0], result.amounts[result.amounts.length - 1], result.path, result.adapters],
      trader.address,
      fee
    )
    outputTokenContract = await ethers.getContractAt('contracts/interface/IERC20.sol:IERC20', tokenOut)
    expect(await outputTokenContract.balanceOf(trader.address)).to.equal(result.amounts[result.amounts.length - 1])
  })
  ...
  it('Router swap matched the query - with permit', async () => {
    ...
    const domainSeparator = ethers.utils.keccak256(
      ethers.utils.defaultAbiCoder.encode(
        ...
        [
          DOMAIN_TYPEHASH,
          ethers.utils.keccak256(ethers.utils.toUtf8Bytes(await inputTokenContract.name())),
          chainId,
        ...
        [
          ...
          ethers.utils.keccak256(
            ethers.utils.defaultAbiCoder.encode(
              ...
              [PERMIT_TYPEHASH, bob.address, YakRouter.address, amountIn, nonce, deadline]
            )
          ),
        ]
      )
    ...
    // Top up trader with starting tokens
    await helpers.topUpAccountWithToken(trader, bob.address, tokenIn, amountIn, fix.PangolinRouter)
    // Call the query
    let result = await YakRouter.findBestPath(amountIn, tokenIn, tokenOut, steps)
    ...
    outputTokenContract = await ethers.getContractAt('contracts/interface/IERC20.sol:IERC20', tokenOut)
    expect(await outputTokenContract.balanceOf(bob.address)).to.equal(result.amounts[result.amounts.length - 1])
  })
  ...
  it('Mix swap can be done to AVAX', async () => {
    ...
    WAVAX = await ethers.getContractAt('contracts/interface/IWETH.sol:IWETH', assets.WAVAX)
    await WAVAX.connect(trader).deposit({ value: amountIn })
    await helpers.topUpAccountWithToken(trader, trader.address, tokenIn, amountIn, fix.PangolinRouter)
    // Query trade
    let result = await YakRouter.findBestPath(amountIn, tokenIn, tokenOut, steps)
    // Approve for input token
  ...
  it('Mix swap can be done from AVAX', async () => {
    ...
    expect(tokenOutBal).to.equal(result.amounts[result.amounts.length - 1])
  })
  ...
  it('Transactions reverts if expected out-amount is not within slippage', async () => {
    ...
    // Query trade
    let result = await YakRouter.findBestPath(amountIn, tokenIn.address, tokenOut.address, steps)
    ...
    await firstAdapter
      ...
      .swap(amountInChange, amountOut, tokenIn.address, tokenOut.address, externalTrader.address)
    let traderTknBal2 = await tokenOut.balanceOf(externalTrader.address)
    expect(traderTknBal2).to.gt(traderTknBal1)
    // Do the swap
    await expect(
      YakRouter.connect(trader).swapNoSplitFromAVAX(
        [result.amounts[0], result.amounts[result.amounts.length - 1], result.path, result.adapters],
        trader.address,
        fee,
        { value: amountIn }
  ...
  it('User gets expected out-amount+positiveSlippage if conditions change positively', async () => {
    ...
    await firstAdapter
      .connect(externalTrader)
      .swap(amountInChange, amountOut, tokenInChange.address, tokenOutChange.address, externalTrader.address)
    ...
    expect(tokenOutBal).to.gt(result.amounts[result.amounts.length - 1])
  })
  ...
  it('User gets expected out-amount within slippage if conditions change negatively', async () => {
    ...
    expect(changeTknBal2).to.gt(changeTknBal1)
    // Do the swap
    const minAmountOut = result.amounts[result.amounts.length - 1]
      .mul(slippageDenominator.sub(slippage))
      .div(slippageDenominator)
    await YakRouter.connect(trader).swapNoSplitFromAVAX(
      [result.amounts[0], minAmountOut, result.path, result.adapters],
      trader.address,
      fee,
      { value: amountIn }
    )
    ...
    expect(traderTknBal3).to.gt(minAmountOut)
  })
  ...
  it('Swap with 3-4 steps', async () => {
    ...
    await fix.ZeroRouter.connect(trader)
    ...
    // Do the swap
    const swap = () =>
      YakRouter.connect(trader).swapNoSplit(
        [result.amounts[0], result.amounts[result.amounts.length - 1], result.path, result.adapters],
        trader.address,
        fee
      )
    ...
    await expect(swap).to.changeTokenBalance(tokenOutContract, trader, expectedOutAmount)
  })
  ...
  it('Optional fee goes to the claimer', async () => {
    ...
    const _path = [fix.tokenContracts.WAVAX, fix.tokenContracts.PNG]
    const _adapters = [adapters.PangolinAdapter.address]
    const feeClaimer = await YakRouter.FEE_CLAIMER()
    await YakRouter.connect(trader).swapNoSplitFromAVAX(
      [_amountIn, _amountOut, _path.map((t) => t.address), _adapters],
      trader.address,
      _fee,
      { value: _amountIn }
    )
