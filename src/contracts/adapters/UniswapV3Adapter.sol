...
contract UniswapV3Adapter is YakAdapter {
    ...
    mapping(address => mapping(address => uint[])) paths;
    ...
    function _query(
        ...
        address _tokenIn, 
        address _tokenOut
    ) internal override view returns (uint quote) {
    ...
    function getBestQuoteAndPool(
        QParams memory params
    ) internal view returns (uint bestQuote, address bestPool) {
        for (uint i; i < feeAmounts.length; ++i) {
            (uint quote, address pool) = getQuoteAndPoolForFee(
                feeAmounts[i], 
                params
            );
            if (quote > bestQuote) {
    ...
    function getQuoteAndPoolForFee(
        uint24 fee, 
        QParams memory params
    ) internal view returns (uint amountOut, address pool) {
        pool = IUniV3Factory(FACTORY).getPool(
            params.tokenIn, 
            params.tokenOut, 
            fee
        );
    ...
    function _swap(
        ...
        QParams memory params = getQParams(_amountIn, _tokenIn, _tokenOut);
        uint amountOut = _underlyingSwap(params, new bytes(0));
        require(amountOut >= _amountOut, "Insufficient amountOut");
        _returnTo(_tokenOut, amountOut, _to);
    }

    function _underlyingSwap(QParams memory params, bytes memory callbackData) internal returns (uint) {
    ...
    function decodeCallbackData(
        bytes memory data
    ) internal pure returns (address trader, uint minAmountOut) {
        assembly {
