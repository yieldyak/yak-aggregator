...
async function getEnv(forkBlockNumber) {
    ...
    const adapterArgs = [
        'DodoV1Adapter', Object.values(dodo.v1.pools), dodo.v1.helper, 335_000,
    ]
    const Adapter = await ethers.getContractFactory("DodoV1Adapter")
...
describe('YakAdapter - DodoV1', function() {
    ...
    describe('Swapping matches query', async () => {
        ...
        it('BTC -> USDC', async () => {
            await checkAdapterSwapMatchesQuery(tkns.WBTC, tkns.USDC, '10')
        })
        it('USDC -> USDT', async () => {
    ...
    it('Gas-estimate is within limits', async () => {
        const options = [
            [ tkns.USDC, tkns.USDT, parseUnits('100', 6) ],
