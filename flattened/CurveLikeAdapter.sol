...
contract CurveLikeAdapter is YakAdapter {
    ...
    function _query(
        ...
        if (ICurveLikePool(pool).paused() || _amountIn == 0) {
            return 0;
        }
        // Cancel query if tokens not in the pool
        if (!TOKENS_MAP[_tokenIn] || !TOKENS_MAP[_tokenOut]) {
            return 0;
        }
        uint256 tokenIndexIn = ICurveLikePool(pool).getTokenIndex(_tokenIn);
        uint256 tokenIndexOut = ICurveLikePool(pool).getTokenIndex(_tokenOut);
        uint256 amountOut = ICurveLikePool(pool).calculateSwap(uint8(tokenIndexIn), uint8(tokenIndexOut), _amountIn);
        return amountOut;
    }
