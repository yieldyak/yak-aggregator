...
module.exports = async ({ getNamedAccounts, deployments }) => {
  ...
  console.log('YalRouter deployment arguments: ', [ADAPTERS, TRUSTED_TOKENS, FEE_CLAIMER])
  log(`V0)YakRouter`)
  const deployResult = await deploy('YakRouterV0', {
    from: deployer,
    contract: 'YakRouter',
    gas: 4000000,
    args: [ADAPTERS, TRUSTED_TOKENS, FEE_CLAIMER],
    libraries: {
  ...
  let allAdaptersIncluded =
    ADAPTERS.length == currentAdapters.length && ADAPTERS.every((a) => currentAdapters.includes(a))
  ...
  let allTrustedTknAdded =
    TRUSTED_TOKENS.length == currentTrustedTokens.length &&
    TRUSTED_TOKENS.every((a) => currentTrustedTokens.includes(a))
  if (!allTrustedTknAdded) {
    // Add trusted tokens
    console.log('Adding trusted tokens:', TRUSTED_TOKENS.join('\n\t- '))
    await yakRouter
      .connect(deployerSigner)
      .setTrustedTokens(TRUSTED_TOKENS)
      .then((r) => r.wait(2))
  }
